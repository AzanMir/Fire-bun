...
import {
  ...
  UserCircle, Flame, ChevronLeft, ChevronRight, X,
} from "lucide-react";
...
function NavLinks({ collapsed, onLinkClick }) {
      ...
      >
        ...
        <div className="flex h-16 items-center justify-between border-b border-white/10 px-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-lg font-bold tracking-wide">FIRE</span>
          </div>
          <button
            onClick={() => setMobileOpen(false)}
        ...
        <NavLinks collapsed={false} onLinkClick={() => setMobileOpen(false)} />
      </aside>

      {/* ── Desktop sidebar ──────────────────────────────────── */}
      <aside
        className={cn(
          "relative hidden lg:flex h-screen flex-col bg-neutral-900 text-white transition-all duration-300 shrink-0",
          collapsed ? "w-16" : "w-64"
        )}
      >
        {/* Logo */}
