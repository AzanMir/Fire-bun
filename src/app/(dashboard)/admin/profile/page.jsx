...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (user) {
      getProfile(user.id)
        .then((p) => { setProfile(p); setForm({ full_name: p.full_name || "", phone: p.phone || "" }); })
        .catch(console.error)
  ...
  async function handleChangePassword(e) {
    e.preventDefault();
    if (pwForm.newPw !== pwForm.confirm) { toast({ title: "Passwords do not match", type: "error" }); return; }
    if (pwForm.newPw.length < 8) { toast({ title: "Password too short", description: "Minimum 8 characters", type: "error" }); return; }
    setChangingPw(true);
    try {
        ...
        <CardContent>
          <form onSubmit={handleSaveProfile} className="space-y-4">
            ...
            <Button type="submit" disabled={saving} className="bg-orange-500 hover:bg-orange-600">
              {saving ? <><Spinner size="sm" className="mr-2" />Saving...</> : "Save Profile"}
            </Button>
          </form>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <form onSubmit={handleChangePassword} className="space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input type="password" value={pwForm.confirm} onChange={(e) => setPwForm({ ...pwForm, confirm: e.target.value })} placeholder="Repeat password" />
            </div>
            <Button type="submit" disabled={changingPw} variant="outline">
              {changingPw ? <><Spinner size="sm" className="mr-2" />Updating...</> : "Update Password"}
            </Button>
          </form>
        </CardContent>
      </Card>
